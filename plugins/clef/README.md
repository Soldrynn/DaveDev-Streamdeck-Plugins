...
1. Download `com.davedev.clef.streamDeckPlugin` from the
   [latest GitHub release](https://github.com/Soldrynn/DaveDev-Streamdeck-Plugins/releases/latest).
...
If the issue continues, [open a GitHub issue](https://github.com/Soldrynn/DaveDev-Streamdeck-Plugins/issues/new)
...
From the `plugins/clef` directory, run the following commands in order:
...
Clef lives in `plugins/clef` inside the DaveDev Stream Deck plugins repository. The
plugin directory uses the following top-level directories:
...
Use [GitHub issues](https://github.com/Soldrynn/DaveDev-Streamdeck-Plugins/issues) for
